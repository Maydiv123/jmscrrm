...
export default function JobDetailsPage() {
  ...
  async function fetchJobDetails() {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/pipeline/jobs/${params.id}`, {
        credentials: "include"
      });
          ...
          <div className="mb-8">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-4">
                <span className={`inline-flex px-3 py-1 text-sm font-semibold rounded-full ${getStageColor(job.current_stage)}`}>
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-900">{job.stage1.consignee || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Shipper</p>
                      <p className="text-sm text-gray-900">{job.stage1.shipper || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Commodity</p>
                      <p className="text-sm text-gray-900">{job.stage1.commodity || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Weight</p>
                      <p className="text-sm text-gray-900">{job.stage1.weight || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Packages</p>
                      <p className="text-sm text-gray-900">{job.stage1.packages || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Container No</p>
                      <p className="text-sm text-gray-900">{job.stage1.container_no || 'N/A'}</p>
                    </div>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-900">{job.stage2.hsn_code || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Documents Type</p>
                      <p className="text-sm text-gray-900">{job.stage2.documents_type || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Filing Requirement</p>
                      <p className="text-sm text-gray-900">{job.stage2.filing_requirement || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Checklist Sent</p>
                      <p className="text-sm text-gray-900">{formatDate(job.stage2.checklist_sent_date)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Approval Date</p>
                      <p className="text-sm text-gray-900">{formatDate(job.stage2.approval_date)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Bill of Entry No</p>
                      <p className="text-sm text-gray-900">{job.stage2.bill_of_entry_no || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Bill of Entry Date</p>
                      <p className="text-sm text-gray-900">{formatDate(job.stage2.bill_of_entry_date)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Duty Amount</p>
                      <p className="text-sm text-gray-900">{formatCurrency(job.stage2.duty_amount)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Duty Paid By</p>
                      <p className="text-sm text-gray-900">{job.stage2.duty_paid_by || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Ocean Freight</p>
                      <p className="text-sm text-gray-900">{formatCurrency(job.stage2.ocean_freight)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Destination Charges</p>
                      <p className="text-sm text-gray-900">{formatCurrency(job.stage2.destination_charges)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">DRN No</p>
                      <p className="text-sm text-gray-900">{job.stage2.drn_no || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">IRN No</p>
                      <p className="text-sm text-gray-900">{job.stage2.irn_no || 'N/A'}</p>
                    </div>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-900">{formatDate(job.stage3.exam_date)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Out of Charge</p>
                      <p className="text-sm text-gray-900">{formatDate(job.stage3.out_of_charge)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Clearance Expenses</p>
                      <p className="text-sm text-gray-900">{formatCurrency(job.stage3.clearance_exps)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Stamp Duty</p>
                      <p className="text-sm text-gray-900">{formatCurrency(job.stage3.stamp_duty)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Custodian</p>
                      <p className="text-sm text-gray-900">{job.stage3.custodian || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Offloading Charges</p>
                      <p className="text-sm text-gray-900">{formatCurrency(job.stage3.offloading_charges)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Transport Detention</p>
                      <p className="text-sm text-gray-900">{formatCurrency(job.stage3.transport_detention)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Dispatch Info</p>
                      <p className="text-sm text-gray-900">{job.stage3.dispatch_info || 'N/A'}</p>
                    </div>
                  </div>
                  
                  {/* Containers */}
                  {job.stage3.containers && job.stage3.containers.length > 0 && (
                    <div className="mt-4">
                      <h3 className="text-sm font-medium text-gray-900 mb-2">Containers</h3>
                      <div className="space-y-2">
                        {job.stage3.containers.map((container, index) => (
                          <div key={index} className="bg-gray-50 p-3 rounded">
                            <div className="grid grid-cols-2 gap-2 text-sm">
                              <div>
                                <span className="font-medium">Container:</span> {container.container_no || 'N/A'}
                              </div>
                              <div>
                                <span className="font-medium">Size:</span> {container.size || 'N/A'}
                              </div>
                              <div>
                                <span className="font-medium">Vehicle:</span> {container.vehicle_no || 'N/A'}
                              </div>
                              <div>
                                <span className="font-medium">Offloading:</span> {formatDate(container.date_of_offloading)}
                              </div>
                            </div>
                          </div>
                        ))}
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-900">{job.stage4.bill_no || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Bill Date</p>
                      <p className="text-sm text-gray-900">{formatDate(job.stage4.bill_date)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Amount Taxable</p>
                      <p className="text-sm text-gray-900">{formatCurrency(job.stage4.amount_taxable)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">GST 5%</p>
                      <p className="text-sm text-gray-900">{formatCurrency(job.stage4.gst_5_percent)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">GST 18%</p>
                      <p className="text-sm text-gray-900">{formatCurrency(job.stage4.gst_18_percent)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Bill Mail</p>
                      <p className="text-sm text-gray-900">{job.stage4.bill_mail || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Bill Courier</p>
                      <p className="text-sm text-gray-900">{job.stage4.bill_courier || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Courier Date</p>
                      <p className="text-sm text-gray-900">{formatDate(job.stage4.courier_date)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Acknowledge Date</p>
                      <p className="text-sm text-gray-900">{formatDate(job.stage4.acknowledge_date)}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-600">Acknowledge Name</p>
                      <p className="text-sm text-gray-900">{job.stage4.acknowledge_name || 'N/A'}</p>
                    </div>
                  </div>
                </div>
              )}
